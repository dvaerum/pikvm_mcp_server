...
0d. **`pikvm_hid_recover`** - Escalating HID recovery ladder: **R0** presence-gate (screenshot returns an image — nothing recovers an asleep/absent target) → **R1** soft reset → **R2** `soft_connect` toggle → **R3a** UDC rebind → **R3b** reboot (`allowReboot`), verifying **behaviorally** after each (emit move + screen-changed; the online flags lie). Honest reliability: R1 rarely fixes a controller drop; R2/R3a untested; R3b reboot is the most reliable remote option (destructive); if all remote rungs fail it escalates to **R4** — a human must physically re-plug/power the target. R2/R3a/R3b are privileged HOST ops via the pikvm-nixos trigger (`PIKVM_HID_RECOVERY_URL`, actions `{soft_connect,udc-rebind,reboot}`); unavailable until wired. Runbook: `docs/runbooks/hid-recovery.md`.
