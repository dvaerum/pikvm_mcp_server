...
0a. **`pikvm_health_check`** - One-call deployment health report: server version, mouseAbsoluteMode + safety-guard implication, **streamer source state (Phase 189: distinguishes "PiKVM down" from "device behind HDMI is off")**, live HID profile, **ground-truth USB HID gadget state (M4: the kvmd online flags lie — the UDC `/sys/class/udc` state via the recovery endpoint is authoritative; flags a FLAG-LIE mismatch both directions and drives the HID verdict off the UDC state → `pikvm_usb_reconnect` when down)**, **(d) pointer diagnosis: when HID is UP it localizes the cursor and splits a broken HID into "HID DOWN" (input path dead → `pikvm_usb_reconnect`) vs "HID UP but cursor NOT LOCALIZABLE" (gadget attached, pointer faded/off-screen → wake the cursor with `pikvm_mouse_move`, reconnect won't help). A CONFIDENT "HID DOWN → reconnect" directive is emitted only from UDC KERNEL STATE; with no UDC reader wired the kvmd flags are the fallback and a both-offline reading is a NON-DIRECTIVE hedge ("confirm behaviorally before reconnecting") — the flags are known to misreport DOWN on a working HID**, iPad bounds detection, screen brightness. Run FIRST after deployment to verify safety guards are active and the target is what you think it is. Surfaces stale deployments (version mismatch), failed startup detection (mouseAbsoluteMode at safe default), source-side outages (e.g. iPad battery dead → `Streamer source: OFFLINE`), and target type (iPad portrait/landscape vs other). **Run this FIRST when `pikvm_screenshot` returns 503** — it tells you whether the issue is PiKVM or the source device.
...
0d. **`pikvm_hid_recover`** - Escalating HID recovery ladder: **R0** presence-gate (screenshot returns an image — nothing recovers an asleep/absent target) → **R1** soft reset → **R2** `soft_connect` toggle → **R3a** UDC rebind → **R3b** reboot (`allowReboot`), verifying **behaviorally** after each (emit move + confirm the CURSOR re-localizes and moved — not a bare screen diff, so a clock tick/animation can't false-pass "RECOVERED"; the online flags lie). Reliability: R1 rarely fixes a controller drop; **R2 `soft_connect` is VALIDATED (~6s, 2026-07-23) — the primary no-reboot fix**; R3a UDC rebind untested; R3b reboot is the destructive last resort (rarely needed now); if all remote rungs fail it escalates to **R4** — a human must physically re-plug/power the target. R2/R3a/R3b are privileged HOST ops via the pikvm-nixos trigger (`PIKVM_HID_RECOVERY_URL`, actions `{soft_connect,udc-rebind,reboot}`); unavailable until wired. **(d): on failure it names the mode — HID DOWN (dead input path → reboot/re-plug makes sense) vs HID UP but cursor NOT LOCALIZABLE (attached but pointer invisible → wake it, don't reboot).** Runbook: `docs/runbooks/hid-recovery.md`.
...
8. **`pikvm_mouse_scroll`** - Scroll wheel; optional `x,y` targets a pane (moves the pointer there first, then scrolls)
...
16. **`pikvm_mouse_click_at`** - `pikvm_mouse_move_to` + `mouseClick` + click verification (pre/post screenshot diff; returns `screenChanged: true|false`). Use the verdict to detect missed clicks instead of eyeballing screenshots; opt out via `verifyClick: false`. **`singleTap: true` (M6)** = keypad/PIN-pad mode: tap once, verify ADVISORY only, NEVER re-fire on no-change (forces `maxRetries:0`, defaults `minBrightness:0`) — stops the retry loop double-tapping a key whose effect is sub-threshold (one "1" → "11"). **`expectRegion: {x,y,width,height}` (M6)** = scope the verify diff to an explicit HDMI-px box (e.g. the PIN-dots field) so a small legit change registers instead of being diluted to a false "no change"; takes precedence over `verifyRegionHalfPx`, advisory-only (pair with `singleTap`). **`capture` (M8)** — on `pikvm_mouse_click_at`, `pikvm_mouse_move`, `pikvm_mouse_move_to`: pass `capture: ["before","during","after"]` (any subset) + `capturePrefix` (required) + optional `captureRegion{x,y,width,height}` to write `${prefix}-before/during/after.jpg` and get the saved paths+sizes back in the result. `during` is grabbed via the net-zero cursor-alive nudge (guaranteed cursor-rendered; for click_at that's pre-button-down, same point as `PIKVM_PREDOWN_DIR`). Advisory — never alters the click/move, only adds latency for the phases requested. **`force: true`** = escape hatch: when the iPad cursor can't be localized (detection fails), the click is normally NOT performed (not-landed error); `force` sends it anyway at the predicted position and returns a LOUD **UNVERIFIED** result (landing NOT confirmed — never read it as a landing). Default false (truthful skip). Typically pair with `strategy:"assume-at"` + `assumeCursorAtX/Y`. Does not affect the `maxResidualPx` wrong-element gate.
