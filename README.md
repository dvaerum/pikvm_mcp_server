...
- **`pikvm_hid_recover`** - Escalating HID-recovery ladder: detects a broken HID, then climbs rung 1 (soft reset) → rung 2 (UDC rebind) → rung 3 (reboot), verifying mouse+keyboard come back online after each. Honest reliability — rung 1 is a cheap first try that often does **not** fix a controller-level drop; rung 3 (`allowReboot:true`) is the known-reliable fix; rung 2 is the untested no-reboot candidate. Rungs 2-3 are privileged host operations performed via the pikvm-nixos recovery trigger (`PIKVM_HID_RECOVERY_URL`) and are reported unavailable until it is wired. Full runbook + trigger contract: [`docs/runbooks/hid-recovery.md`](docs/runbooks/hid-recovery.md).
