...
- **`pikvm_mover_scale_status`** / **`pikvm_mover_scale_control`** / **`pikvm_mover_scale_reset`** - The passive curve-scale learner (#41). The iPad emit→pixel scale drifts with the detected HDMI region; rather than a hardcoded constant going stale, the mover passively learns `curveScaleX/Y` from every real move's free planned-vs-achieved residual (first-shot only, per-axis, ≥150px gate, rolling median, SE-gated ≤2%/update, clamped [0.85,1.15], warm-started from the shipped defaults, persisted to `$XDG_STATE_HOME/pikvm-mcp/`). **status** reads the per-axis scales/counters/warnings (a constant landing offset ⇒ detector/pacing fault not geometry; >2% divergence ⇒ re-bake candidate); **control** `action:enable|disable` freezes/resumes (env kill-switch `PIKVM_MOVER_LEARN=0`); **reset** reverts to defaults and deletes the persisted file. `maxResidualPx` is untouched.
