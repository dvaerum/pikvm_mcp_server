...
- **`pikvm_mouse_click_at`** - Approximate move + click on a relative-mouse target (required: x, y; optional: button). `singleTap: true` (M6) = keypad/PIN-pad mode: taps once, verification advisory-only, never re-fires on no-change (forces `maxRetries:0`, defaults `minBrightness:0`) — prevents the retry loop from double-tapping a key whose tap effect is sub-threshold. `expectRegion: {x,y,width,height}` (M6) = scope the verification diff to an explicit HDMI-px box (e.g. the PIN-dots field) so a small legit change registers; takes precedence over `verifyRegionHalfPx`, advisory-only.
