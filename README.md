...
- **`pikvm_health_check`** - One-call deployment health: server version + safety-guard state + **streamer source state (Phase 189: distinguishes "PiKVM down" from "device behind HDMI is off")** + live HID profile + **ground-truth USB HID gadget state (M4 — the kvmd online flags lie; the UDC `/sys/class/udc` state via the recovery endpoint is authoritative, flags mismatches both ways, and drives the HID verdict → `pikvm_usb_reconnect` when down)** + **(d) pointer diagnosis (when HID is UP, localizes the cursor and splits "HID DOWN" — input path dead → reconnect — from "HID UP but cursor NOT LOCALIZABLE" — gadget attached, pointer faded/off-screen → wake it, reconnect won't help; a CONFIDENT "HID DOWN → reconnect" directive comes only from UDC kernel state, while the flags-only fallback hedges non-directively because kvmd flags misreport DOWN on a working HID)** + iPad bounds detection + screen brightness. Run FIRST after deployment, AND when `pikvm_screenshot` returns 503 (Phase 190 enriches the error with a hint pointing here). Surfaces stale deployments, failed startup detection, source-side outages (e.g. iPad battery dead), and target-type mismatches.
...
- **`pikvm_hid_recover`** - Escalating HID-recovery ladder: **R0** presence-gate (a screenshot must return an image — nothing recovers an asleep/absent target) → **R1** soft reset → **R2** `soft_connect` toggle → **R3a** UDC rebind → **R3b** reboot (`allowReboot:true`), **verifying behaviorally** after each rung (emit a mouse move + confirm the screen changed — the online flags have lied). Reliability — R1 rarely fixes a controller-level drop; **R2 `soft_connect` is validated (recovered a real idle drop in ~6s, 2026-07-23) — the primary no-reboot fix**; R3a UDC rebind is untested; R3b reboot is the destructive last resort (~30-90s, rarely needed now); when every remote rung fails it escalates to **R4**: a human must physically re-plug the target USB or power it on. On failure it also names the failure mode — **HID DOWN** (input path dead) vs **HID UP but cursor NOT LOCALIZABLE** (attached but pointer invisible → wake it, don't reboot). R2/R3a/R3b are privileged host operations via the pikvm-nixos recovery trigger (`PIKVM_HID_RECOVERY_URL`, actions `{soft_connect, udc-rebind, reboot}`), reported unavailable until wired. Full runbook + trigger contract: [`docs/runbooks/hid-recovery.md`](docs/runbooks/hid-recovery.md).
- **`pikvm_usb_reconnect`** - The everyday "mouse/keyboard died — reconnect the USB" fix (reach for this first). Presence-gates the target, then runs the two validated host rungs — `soft_connect` (idle-drop, ~6s) then, if input is still dead, `udc-rebind` (full-dead HID after a PiKVM reboot) — with **no destructive reboot**. Each rung is verified by BOTH the **ground-truth UDC state** (`GET {PIKVM_HID_RECOVERY_URL}/udc-state`) AND a behavioral check (emit + screen-changed), because the kvmd HID online flags lie. Returns structured `{recovered, rungUsed, udcState, behavioralVerify, attempts, message}`. On failure it appends a **Diagnosis** line distinguishing **HID DOWN** (dead input path — reconnect/reboot is right) from **HID UP but cursor NOT LOCALIZABLE** (gadget attached, pointer faded/off-screen — wake the cursor instead), so you don't reboot for a problem a reboot can't fix. If it still fails, escalate with `pikvm_hid_recover` (adds the reboot rung + human step). Needs the pikvm-nixos recovery endpoint configured.
...
- **`pikvm_hidmode_status`** / **`pikvm_hidmode_set`** - The HID-mode surface (#51). The appliance can switch its HID mode at runtime (desktop = absolute dual mouse; ipad = single relative mouse); it owns the mode and the MCP derives its own relative/absolute behaviour from it, holding no second copy. **status** reports the source (`declared` — a fixed `--target`, e.g. stock pikvm01 — vs `endpoint` — derived from the appliance `PIKVM_HIDMODE_URL`/hidmode), the current mode (or `null`=UNKNOWN when the endpoint is unreachable), reachability, settling (HID mid-switch), and whether pointer ops are allowed. **set** `{mode}` switches the appliance (POSTs `/hidmode`; a declared target is fixed and cannot be switched) — the switch **drops the session** and the new mode is **not live** until the USB re-enumerates, so reconnect + re-read before driving input. When derived and the endpoint is unreachable, pointer ops **fail closed** (refuse) rather than guess the mode. See `docs/adr/0002-mcp-derives-hid-mode-from-appliance-endpoint.md`.
- **`pikvm_mover_scale_status`** / **`pikvm_mover_scale_control`** / **`pikvm_mover_scale_reset`** - ⚠️ **EXPERIMENTAL, OFF BY DEFAULT** (#41). The passive curve-scale learner. The iPad emit→pixel scale drifts with the detected HDMI region; this experiment tries to learn `curveScaleX/Y` from every real move's free planned-vs-achieved residual (first-shot only, per-axis, ≥150px gate, rolling median, SE-gated ≤2%/update, warm-started from the shipped defaults, persisted to `$PIKVM_STATE_DIR/data/`). **Opt in per-process with `PIKVM_MOVER_LEARN=1`** — when not opted in (the default), the learner is inert, these 3 tools are **not registered**, and the mover uses the static shipped default (a true no-op). **Known limitation** (real-rig verified): the auto-adaptation does not reliably beat a one-time human measurement (median ~1% biased; the unbiased slope estimator wanders ±2-3% on the rig's two-cluster traffic), so it ships the stable median **clamped to ±1% of the shipped default** and is provided for evaluation, not production-trusted; the drift DETECTION/warnings are the more reliable half. **status** reads per-axis applied scale + unclamped estimate + counters/warnings (constant landing offset ⇒ detector/pacing fault not geometry; >2% estimate divergence ⇒ re-bake candidate); **control** `action:enable|disable` freezes/resumes within the opted-in session; **reset** reverts to defaults and deletes the persisted file. `maxResidualPx` is untouched. See `docs/learnings/passive-scale-learner.md`.
...
- **`pikvm_mouse_click_at`** - Approximate move + click on a relative-mouse target (required: x, y; optional: button). `singleTap: true` (M6) = keypad/PIN-pad mode: taps once, verification advisory-only, never re-fires on no-change (forces `maxRetries:0`, defaults `minBrightness:0`) — prevents the retry loop from double-tapping a key whose tap effect is sub-threshold. `expectRegion: {x,y,width,height}` (M6) = scope the verification diff to an explicit HDMI-px box (e.g. the PIN-dots field) so a small legit change registers; takes precedence over `verifyRegionHalfPx`, advisory-only. `capture` (M8) — on `pikvm_mouse_click_at`/`pikvm_mouse_move`/`pikvm_mouse_move_to`: `capture: ["before","during","after"]` (any subset) + `capturePrefix` (required) + optional `captureRegion{x,y,width,height}` writes `${prefix}-before/during/after.jpg` (paths+sizes returned). `during` uses the cursor-alive nudge (guaranteed cursor-rendered; pre-button-down for click_at). Advisory — never alters the operation. `force: true` = escape hatch: when the iPad cursor can't be localized, the click is normally skipped (not-landed error); `force` clicks anyway at the predicted position and returns a LOUD UNVERIFIED result (landing not confirmed). Default false; pair with `strategy:"assume-at"`.
