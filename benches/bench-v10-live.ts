/**
 * Live A/B click bench: v10 (ml/cursor-v10.onnx, trained on YOUR human
 * labels) vs the deployed pipeline (still v9-bordered).
 *
 * Set PIKVM_ML_V8_MODEL=ml/cursor-v10.onnx (we set it in this script)
 * so the production click pipeline uses v10 for cursor detection.
 *
 * Each trial:
 *  1. ipadGoHome + settle
 *  2. Take pre-click screenshot ("the frame v10 would see")
 *  3. Run v10 inference directly on that frame, record its predicted (x, y)
 *  4. Run v9-bordered inference too (for reference)
 *  5. Execute the click via clickAtWithRetry (which will use v10 because
 *     of the env var)
 ...
 *     - pre-click frame
 *     - post-click frame
 *     - JSON sidecar with target, v10 pred, v9 pred, click result, classification
 *
 * Output: data/cursor-collect-v10-livebench-{TS}/
 *   {scene}/frame-NNN-pre.jpg
 *   {scene}/frame-NNN-post.jpg
 *   {scene}/frame-NNN.json
 *   verified-for-review.jsonl   (for label-review tool — v10 pred is the algo arrow)
 *   home-reference.jpg
 *   summary.json
 *
 * Usage:
 *   npx tsx bench-v10-live.ts [trials_per_target=5]
 */

// Set env BEFORE importing anything that reads it.
process.env.PIKVM_ML_V8_MODEL = 'ml/cursor-v10.onnx';
...
async function main() {
  ...
  const ROOT = `./data/cursor-collect-v10-livebench-${ts}`;
  await fs.mkdir(ROOT, { recursive: true });
  console.error(`v10 live bench: PIKVM_ML_V8_MODEL=${process.env.PIKVM_ML_V8_MODEL}`);
  ...
  const v10Session = await ort.InferenceSession.create('ml/cursor-v10.onnx');
  const v9Session = await ort.InferenceSession.create('ml/cursor-v9-bordered.onnx');
  console.error('loaded v10 + v9-bordered ONNX for per-trial prediction logging');
  ...
  for (const t of TARGETS) {
    ...
    for (let i = 1; i <= TRIALS; i++) {
      ...
      // 2. Pre-click screenshot (the frame v10 will see after the click pipeline takes its own).
      ...
      // 3. Run v10 + v9 directly for prediction comparison.
      const v10Pred = await predictFullFrame(v10Session, preShot.buffer);
      const v9Pred = await predictFullFrame(v9Session, preShot.buffer);

      // 5. Click via the production pipeline (which will use v10 via env var).
      ...
      const sidecar = {
        ...
        classification: cls,
        sim_to_home: sim,
        v10_prediction: { x: Math.round(v10Pred.x), y: Math.round(v10Pred.y), presence: v10Pred.presence },
        v9_prediction: { x: Math.round(v9Pred.x), y: Math.round(v9Pred.y), presence: v9Pred.presence },
        v10_v9_disagreement_px: Math.round(Math.hypot(v10Pred.x - v9Pred.x, v10Pred.y - v9Pred.y)),
      ...
      const reviewEntry = JSON.stringify({
        ...
        cursor: null,  // YOU label this — was v10's pick correct?
        scene: `${t.slug}:${cls}`,
        algorithm_label: { x: Math.round(v10Pred.x), y: Math.round(v10Pred.y) },
        v9_label: { x: Math.round(v9Pred.x), y: Math.round(v9Pred.y) },
        target: { x: t.x, y: t.y, name: t.name },
        classification: cls,
        v10_v9_disagreement_px: sidecar.v10_v9_disagreement_px,
      });
      ...
      console.error(
        ...
        `v10=(${Math.round(v10Pred.x)},${Math.round(v10Pred.y)}) ` +
        `v9=(${Math.round(v9Pred.x)},${Math.round(v9Pred.y)}) ` +
        `Δ=${sidecar.v10_v9_disagreement_px}px sim=${sim.toFixed(3)}`,
      );
    }
  }
