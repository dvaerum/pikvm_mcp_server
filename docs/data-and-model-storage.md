...
- **T1 SEED — irreplaceable raw ingredients → COMMIT to git (all small):**
  ...
  app backgrounds captured live), and the eval-gate frames (now `data/seeds/eval-frames/`).
  Cannot be regenerated without the iPad. (The old `human-verified*.jsonl` labels belonged to the
  RETIRED full-frame models; the current cascade does not use them, so per the user they were
  DELETED 2026-07-20.)
...
4. **Deleted** (~13.4 GB reclaimed): `data/scene-backgrounds/` (8.3 GB), all `cursor-collect-*`
   ...
   on-disk models. Preserved: the seeds above. (The 40 human-label jsonls were initially kept, then
   DELETED per the user — the current cascade does not use them.)
...
3. **Legacy full-frame corpora + models** — DONE: dropped the regenerable/legacy frames + 371 MB of
   models. The human-label jsonls were briefly kept, then DELETED per the user (2026-07-20) — the
   current cascade needs none of them (proven live at N=80 with everything else quarantined).
