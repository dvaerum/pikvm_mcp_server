...
| — | `slam_to_corner` verification-screenshot 503s | **ROOT CAUSE DECISIVELY CONFIRMED — the display, not the transport** | Blocked categories 2/5's completion; now fully understood. This turned out to be TWO separate, real findings layered together: (1) a genuine WS-zombie-connection bug in `StreamerKeepalive` (held connection never actively probed liveness) — fixed by `docs/streamer-keepalive-liveness-ping-plan.md`'s active Ping/Pong (35e64ba, 966/966 tests) — real, worth keeping, but proven NOT the driver of this specific 503 pattern (live-verified: the fix's own connection stayed provably alive through a 32s idle hold while the screenshot still 503'd). (2) The ACTUAL cause, found via a careful multi-step elimination (REST-request-recency ruled out; "any new connection event" ruled out via a genuinely separate, independent `StreamerKeepalive` that connected fine but didn't revive the original client; a real self-caught confound along the way — every earlier "fresh client recovers it" observation had also, unnoticed, sent a wake keypress): **a single `Space` keypress through the SAME already-stuck client immediately revived `source.online` and let the screenshot succeed.** This was never a connection-tracking bug — the iPad's own display goes properly dark during the long human-confirmation wait (consistently ~10.6-10.7s across 3 separate live measurements) and ustreamer correctly reports no capture source until a real redraw event occurs; nothing server-side can substitute for that. **Unifies with the wake-key delay-sweep's own earlier finding** (short delay before `Space` → reliably escalates to Touch ID; long delay, ~8s+ → reliably stays clean) — likely the SAME underlying ~10s-ish display timing, studied from two different angles across this session. **Fix, not yet designed**: a periodic or well-timed wake key during any long human-confirmation wait, reusing the same single-press discipline already established (a second/repeated `Space` on an already-lit screen can itself escalate to Touch ID — this is not "just send more wake keys," it needs the same care as everything else learned about this mechanism today). |
...
2. **Category 2 positive+negative control pair actually run and passed —
   NOT YET, but the real root cause is now decisively known.** 6 real
   live attempts were blocked at `slam_to_corner`'s verification-
   screenshot 503s (see §1's inventory row) — not the guard/slam logic
   itself, believed sound throughout. Two real, separate causes: the
   `StreamerKeepalive` ping/pong fix (35e64ba) closes a genuine
   zombie-connection bug but was live-confirmed NOT to be what drove
   this specific pattern; the actual cause is the iPad's own display
   going dark during the long confirmation wait (~10.6-10.7s
   consistently), fixed only by a real wake event — confirmed by a
   single `Space` keypress reviving an already-stuck client instantly.
   Next step is designing a safe periodic-wake mechanism for the
   confirmation wait (reusing the established single-press discipline,
   not a naive repeat), THEN a fresh category 2 attempt.
