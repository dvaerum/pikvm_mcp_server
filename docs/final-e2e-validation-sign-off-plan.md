...
| 5 | `ipad_unlock.rs`'s `CallerAsserted`-on-lock-screen positive path | **STILL OPEN — a second real attempt, genuinely reached the guard, still no verified result** | Checked directly against the real code before reporting either way: `unlock_ipad()` DOES have a real internal `CallerAsserted`-on-lock-screen call site (`unlock.rs:228`, gated behind `slam_first`, default true) — a genuine, reachable production path, not hypothetical. Run #7 didn't reach it (key-press-first recovery succeeded and returned early — `recovery slam_verified: None` proves it). **Run #8 (2026-08-30)** forced the reachable path directly (`unlock_ipad(try_key_press_first: false)`) — the guard WAS genuinely reached this time (the slam's own log line confirmed `anchor_cursor` entered `slam_to_corner`, correctly not refusing) — but the internal slam's own `before` screenshot hit the same `source.online` pattern and exhausted its retry, because `unlock_ipad()`'s own internal `AnchorRequest` explicitly sets both `allow_keyboard_wake` fields `false` (never one of the two approved sites). `unlock_ipad()` propagated the error; zero HID near a corner; device confirmed safe afterward. **Calibration, explicit**: "guard reached, didn't refuse" is real, positive evidence the precondition logic is sound that far — but it is NOT "reached and passed" (this item's own literal wording). No verified result was ever produced, so this remains a non-result, not a pass, same standard as category 2's own bar. **Reachable next step, not yet built**: extend the keyboard-wake escalation to `unlock_ipad()`'s own internal slam (`try_key_press_first: false` config) — same causal argument already approved for the two corner-control-smoke sites, likely the actual, final unlock for this item once reviewed and built. |
