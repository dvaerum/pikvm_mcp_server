...
| — | `slam_to_corner` verification-screenshot 503s (transport gap) | **FIX SHIPPED (35e64ba) AND CONFIRMED INSUFFICIENT ALONE — a 2nd, separate cause identified** | Blocks categories 2/5's actual completion. First cause (real, confirmed, now fixed): the held `/api/ws` connection dropped its write half and never actively probed liveness — `docs/streamer-keepalive-liveness-ping-plan.md`'s active Ping/Pong fix closes this (implemented + reviewed, 966/966 workspace tests). **Live verification of the fix (2026-08-30) gave an honest negative result**: `streamer_keepalive_connected=true` held throughout a 32s idle hold (the ping/pong is working at that layer) — but the post-idle screenshot STILL 503'd. This means the WS-zombie-connection gap was real but is NOT the only cause of tonight's 503 pattern. **2nd lead, not yet confirmed**: connects to an earlier finding this session that `/api/streamer`'s `source.online` field gave a false reading, root-caused then to "ustreamer's own on-demand run state" (process-level cycling), unrelated to the iPad's actual lock/screen state — suggesting ustreamer's own capture-source layer may independently 503 regardless of WS-session health, which a client-side keepalive fix structurally cannot reach. **Next diagnostic (deliberately deferred to a fresh session, not chased further tonight)**: log `/api/streamer`'s `source.online` directly alongside `streamer_keepalive_connected` at the moment of a screenshot failure, correlated in time — but `source.online`'s own established unreliability means a single correlated reading won't be conclusive either; this needs careful characterization, not a quick guess. The ping/pong fix stands on its own merits regardless of this 2nd cause — not walked back. |
...
2. **Category 2 positive+negative control pair actually run and passed —
   NOT YET, a 2nd independent cause was found after the first fix
   shipped.** 6 real live attempts were blocked at `slam_to_corner`'s
   verification-screenshot 503s (see §1's inventory row) — not the
   guard/slam logic itself, believed sound throughout. The first
   root-cause fix (`StreamerKeepalive` ping/pong, commit 35e64ba) is
   implemented, reviewed, AND live-verified to be working correctly at
   its own layer (connection stayed provably alive through a 32s idle
   hold) — but the verification screenshot still 503'd anyway, revealing
   a 2nd, separate cause (likely ustreamer's own capture-source layer,
   not yet confirmed — see §1). Next step is characterizing that 2nd
   cause in a fresh session, THEN a fresh category 2 attempt — not
   re-attempting the guarded slam against a still-only-partially-
