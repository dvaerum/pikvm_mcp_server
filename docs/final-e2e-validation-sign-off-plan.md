...
2. **Category 2 positive+negative control pair actually run and passed —
   NOT YET, but the fix is now fully implemented for this call site.**
   ...
   itself, believed sound throughout. Root cause: the iPad's own display
   ...
   default false everywhere, `true` only for this harness's AFTER
   screenshot, reviewed safety argument in
   `docs/corner-control-allow-keyboard-wake-decision.md`, commit
   dfdf18c). Next step is this specific fix's own live verification,
