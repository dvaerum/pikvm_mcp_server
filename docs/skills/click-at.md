...
| useKnownFpBlocklist | boolean | false | Phase 248/249 (v0.5.213/v0.5.214): when true, reject template-match cursor positions in 50 px of known UI false-positive locations on the reference iPad (1680×1050 portrait, default wallpaper). **Honest data:** Phase 248 N=60 (3×20): runs 40%/5%/35% — cumulative 26.7% vs baseline 25% (~1.7 pp lift, within Phase 237 variance). Single-N=20 swings wildly. Motion-diff results aren't filtered by the blocklist. **Only enable if your iPad layout matches the reference** — different wallpaper/layout has different FPs. |
...
```
Phase 248/249 opt-in. Rejects template matches at 3 known iPad-UI false-positive locations (wallpaper-gradient FP at (852,941), TV icon glyph at (773,769), dock area at (782,958)). **Cumulative N=60 shows minimal lift** (26.7% vs 25% baseline, within variance) — semantically correct rejection without proven hit-rate lift. May still be useful when callers specifically want to avoid landing at known-bad positions. Don't enable on a different iPad layout / wallpaper — the FPs may not be the same.
