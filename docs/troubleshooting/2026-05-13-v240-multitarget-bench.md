...
**Status:** Detection layer verified across 4 targets. Click ceiling cause not established. (Earlier framing "at iPad pointer-effect" is on the REJECTED_CLAIMS.md list as unverified.)
...
|----------|--------|--------|-------|--------------------------------------|
| Settings | 5      | 5/5    | 0/5   | 3× residual ≈19 px (earlier framing "cursor on icon, iPad ignored tap — Phase 310" is on the REJECTED_CLAIMS.md list as unverified; the reported residual is the detector's self-report) |
...
| AppStore | 5      | 4/5    | 1/5   | 1 NULL, 2× residual 3-8 px (earlier "tap ignored" framing on REJECTED_CLAIMS.md as unverified), 1 success at residual 70 px |
...
- **5 trials with residual ≤ 20 px** (Settings 19×3, AppStore 3, 8):
  the detector reported low residuals. (Earlier framing called
  these "cursor dead-on icon" cases; that takes the detector's
  self-report as ground truth — see REJECTED_CLAIMS.md re
  tautological residuals.) None registered a successful click.
  Cause not established. (Earlier framing "Phase 310 tautology /
  pointer-effect snap zone consumes the tap" is on the
...
The detection improvement is solid. The click rate ceiling cause is
not established. (Earlier framing "iPad-side and outside the
detection layer's control" is on the REJECTED_CLAIMS.md self-stop
list as unverified.)

## Trial-by-trial breakdown

```
Settings r1.1: residual=19px   click=✗  (low reported residual; "tap ignored" framing rejected — see REJECTED_CLAIMS.md)
Settings r1.2: residual=119px  click=✗
Settings r1.3: residual=19px   click=✗  (low reported residual)
Settings r1.4: residual=19px   click=✗  (low reported residual)
...
Books    r1.5: residual=37px   click=✗
...
AppStore r1.2: residual=8px    click=✗  (low reported residual)
AppStore r1.3: residual=3px    click=✗  (low reported residual)
...
2. **Click failures at low reported residual** — 5 trials with
   residual ≤ 20 px ALL failed. (Earlier framing: "the cursor is
   unambiguously on the icon; the iPad consumes the tap"; both
   parts are on the REJECTED_CLAIMS.md list — reported residual
   is the detector's self-report, and "iPad consumes the tap" is
   an unverified mechanism.) No detection change can fix this
   pattern; cause not established.
...
The detection layer is at a ceiling set by upstream issues whose
location is not established. (Earlier framing "the upstream
iPad-side issues" is on the REJECTED_CLAIMS.md self-stop list.)
Next bottleneck options (need explicit user direction):
