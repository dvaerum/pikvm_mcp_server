"""
Train v12.1 — MobileNetV3 cursor detector. v12.1 = v12 train data
+ the larger 2026-05-31 absent-balanced synth collection (roadmap 2.2).
External val replaced with the matching 2026-05-31 val collection,
which was captured with --exclude-list against the train scenes so
there is zero scene overlap between train and external val.

Output: ml/cursor-v12.1.pt + ml/cursor-v12.1.onnx
"""
import json
import random
...
SOURCES = {
    ...
    # pa26 dropped: its frames live under data/click-bench-prod/, which today's
    # bench-click-production runs (3.5 + 3.5-verify + Books re-aim) rm -rf'd
    # and rewrote with different per-trial verdicts. Re-locating 60 frames not
    # worth it when v12.1's new 2855 frames already swamp the corpus.
    "synthetic-train":      DATA_ROOT / "cursor-collect-synthetic-2026-05-30T16-46-52" / "verified.jsonl",
    "synthetic-train-v12.1":DATA_ROOT / "cursor-collect-synthetic-2026-05-31T14-20-38" / "verified.jsonl",
}
...
EXTERNAL_VAL_SOURCES = {
    "synthetic-val-v12.1": DATA_ROOT / "cursor-collect-synthetic-2026-05-31T18-24-40" / "verified.jsonl",
}
...
def load_sources(sources: dict = SOURCES) -> list:
    ...
    for name, jsonl_path in sources.items():
        ...
        for fid, entry in latest.items():
            ...
            if cursor is None:
                # bench-collect-synthetic writes literal `cursor: null` for
                # absent frames; the older human-verified format wrote
                # `cursor: {"visible": false, ...}`. Normalize to the latter
                # so absent rows train the presence head.
...
def main():
    ...
    val_manifest = OUT_DIR / "cursor-v12.1-val-manifest.jsonl"
    ...
    for epoch in range(EPOCHS):
        ...
        if combined < best_combined:
            ...
            torch.save(model.state_dict(), OUT_DIR / "cursor-v12.1.pt")
