"""
Train v12 — MobileNetV3 cursor detector. v12 = v11 + one extra source:
the new synthetic auto-labeled dataset from bench-collect-synthetic.ts.

The synthetic dataset's labels come from the iPad app's pointer system
(not human review), but the row schema is identical to v11's other
sources (frame, cursor:{visible,x,y}, decision, scene, decided_at).
The synth rows additionally carry a `logical:{x,y}` field which the
existing data loader ignores (it only reads `decision` and `cursor`).

Hypothesis: synthetic auto-labels expand positional + scene diversity
cheaply, which should improve generalization without harming v11's
existing presence/absent balance. Risk: synthetic labels are noisier
than human review, so med_dist may rise slightly before falling.

Output: ml/cursor-v12.pt + ml/cursor-v12.onnx
"""
import json
import random
...
def main():
    ...
    for epoch in range(EPOCHS):
        ...
        if combined < best_combined:
            ...
            torch.save(model.state_dict(), OUT_DIR / "cursor-v12.pt")
