...
# Multiple data sources. Each (dir, verified.jsonl-path). Frame .jpg
# files for that source live in dir.
DATA_SOURCES = [
    (
    ...
    (
        ROOT / "data" / "ml-positives-collect",
        ROOT / "data" / "ml-positives-collect" / "verified.jsonl",
    ),
]
OUT_DIR = ROOT / "ml"
...
def load_verified() -> list:
    """Load all sources, annotating each row with its source dir so
    the dataset can resolve the frame's location."""
    rows = []
    for data_dir, jsonl_path in DATA_SOURCES:
        ...
        with open(jsonl_path) as f:
            for line in f:
                ...
                rows.append(row)
                n += 1
        print(f"  loaded {n} rows from {jsonl_path}")
    return rows
...
class CursorDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        src_dir = row.get("_source_dir")
        if src_dir is not None:
            img_path = Path(src_dir) / frame_name
        else:
            img_path = DATA_SOURCES[0][0] / frame_name
...
def main():
    ...
    for epoch in range(EPOCHS):
        ...
        if combined < best_combined:
            ...
            torch.save(model.state_dict(), OUT_DIR / "cursor-v3.pt")
            try:
                export_onnx(model, OUT_DIR / "cursor-v3.onnx")
                print(
