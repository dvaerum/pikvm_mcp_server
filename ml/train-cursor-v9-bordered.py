"""
Train v9-bordered — same architecture as v8, retrained on bordered-cursor
frames after iPadOS Pointer Colour was switched to orange with white border
and pointer size bumped up. The previous v8 hallucinates on these frames
because it was trained on the translucent borderless cursor.

Single source: data/cursor-collect-2026-05-27T19-00-08/human-verified.jsonl
(auto-labeled by combining "is orange" AND "moved between consecutive
frames"; 6/6 spot-checks correct, 3 frames marked absent).

Output: ml/cursor-v9-bordered.pt + ml/cursor-v9-bordered.onnx
"""
import json
import random
...
OUT_DIR = ROOT / "ml"
TEST_FILE = ROOT / "docs" / "ml-architecture-bench" / "TEST_FRAMES.json"

BORDERED_DIRS = [
    ROOT / "data" / "cursor-collect-2026-05-27T19-00-08",
    ROOT / "data" / "cursor-collect-2026-05-27T19-29-10",
    ROOT / "data" / "cursor-collect-2026-05-27T19-32-25",
]
SOURCES = {
    f"bordered-r{i+1}": {
        "verified": d / "human-verified.jsonl",
        "base_dir": d,
    }
    for i, d in enumerate(BORDERED_DIRS)
}

INPUT_W, INPUT_H = 768, 480
HEATMAP_W, HEATMAP_H = INPUT_W // 4, INPUT_H // 4
GAUSSIAN_SIGMA = 4.0  # 2026-05-27: bumped 2→4 because auto-labels have ~5-10 px noise; tight gaussian + noisy labels broke v9-bordered's first run (predicted (0,0))
BATCH_SIZE = 16
LR = 1e-3
EPOCHS = 40
VAL_FRACTION = 0.2
...
def load_test_keys() -> set:
    # No locked TEST split for the bordered-cursor dataset — return empty.
    return set()


def load_sources(test_keys: set) -> list:
    rows = []
    n_total_per_source = {}
    n_test_excluded_per_source = {}
    n_train_per_source = {}

    for name, cfg in SOURCES.items():
        latest = load_latest_per_frame(cfg["verified"])
        n_total = 0
        n_test = 0
        n_kept = 0
        for fid, entry in latest.items():
            ...
            if cursor is None:
                continue  # ambiguous pre-fix confirm
            n_total += 1
            if (name, fid) in test_keys:
                n_test += 1
                continue
            abs_path = cfg["base_dir"] / fid
            rows.append({
                "source": name,
                "frame_id": fid,
            ...
            n_kept += 1
        n_total_per_source[name] = n_total
        n_test_excluded_per_source[name] = n_test
        n_train_per_source[name] = n_kept

    for name in SOURCES:
        print(
            f"{name}: total usable={n_total_per_source[name]}, "
            f"excluded as TEST={n_test_excluded_per_source[name]}, "
            f"kept for train+val={n_train_per_source[name]}"
        )
    vis = sum(1 for r in rows if r["cursor"]["visible"])
    print(f"Total rows: {len(rows)} (visible: {vis}, absent: {len(rows) - vis})")
    return rows
...
def main():
    ...
    print(f"TEST keys to exclude: {len(test_keys)}")
    rows = load_sources(test_keys)
    if not rows:
        print("no training rows; aborting")
        return
    train_rows, val_rows = stratified_split(rows, VAL_FRACTION, SEED)

    val_manifest = OUT_DIR / "cursor-v9-bordered-val-manifest.jsonl"
    ...
    for epoch in range(EPOCHS):
        ...
        if combined < best_combined:
            ...
            torch.save(model.state_dict(), OUT_DIR / "cursor-v9-bordered.pt")
