...
def featurize(r):
    # INVERSE: features = (cursor_pos, DESIRED pixel displacement)
    # We use observed_dx/dy as the "desired" because that's what
    # the emit actually produced. So the model learns:
    #   "if you want this much displacement, here's the emit
    #   that achieved it in similar conditions."
    cx = r["pre_pred"]["x"] / 1680.0
    cy = r["pre_pred"]["y"] / 1050.0
    desired_dx = r["observed_dx"] / 100.0
    desired_dy = r["observed_dy"] / 100.0
    desired_mag = (r["observed_dx"]**2 + r["observed_dy"]**2)**0.5 / 200.0
    if r["observed_dx"] == 0 and r["observed_dy"] == 0:
        rad = 0.0
    else:
        rad = np.arctan2(r["observed_dy"], r["observed_dx"])
    return np.array([cx, cy, desired_dx, desired_dy, desired_mag,
                     np.sin(rad), np.cos(rad)], dtype=np.float32)


def label(r):
    # INVERSE: labels = (emit_dx, emit_dy) in mickeys
    ex = r["emit"]["dx"] / 100.0
    ey = r["emit"]["dy"] / 100.0
    return np.array([ex, ey], dtype=np.float32)
...
def main():
    ...
    # INVERSE baseline: predict emit = desired_displacement / 1.3
    # i.e., the algorithm's current closed-form: emit_mickeys = desired_pixels / ratio.
    val_baseline_l2 = 0.0
    n = 0
    for r in val_rows:
        pred_emit_x = r["observed_dx"] / 1.3
        pred_emit_y = r["observed_dy"] / 1.3
        true_emit_x = r["emit"]["dx"]
        true_emit_y = r["emit"]["dy"]
        val_baseline_l2 += (
            (pred_emit_x - true_emit_x) ** 2 +
            (pred_emit_y - true_emit_y) ** 2
        ) ** 0.5
        n += 1
    val_baseline_l2 /= max(1, n)
    print(
        f"INVERSE baseline (constant 1.3 ratio): {val_baseline_l2:.1f} "
        f"mickey-units RMS"
    )
    ...
    for epoch in range(EPOCHS):
        ...
        if val_loss < best_val:
            best_val = val_loss
            torch.save(model.state_dict(), OUT / "emit-mlp-inverse.pt")
