...
//!
//! **Provisional `MovePassDiagnostic`**: `move-to.ts`'s own `types.ts`-
//! equivalent split (per docs/rust-port-plan.md's move-to.ts decomposition
//! note) puts this type in a shared `move_to::types` module owned by
//! whoever assembles `legacy_move.rs`/`origin.rs` (built in parallel on a
//! separate branch — see this crate's `move_to.rs` header). Defined here,
//! faithfully mirroring the TS shape, so this file is independently
//! buildable/testable now; expected to be superseded by a `pub use` of
//! the shared type once both sides land and are reconciled via branch
//! merge, not a permanent duplicate.

/// Faithful port of `MovePassDiagnostic` (`move-to.ts` lines 359-377).
...
pub struct MovePassDiagnostic {
    ...
    /// score, or "ok"). `None` mirrors the TS `null`.
...
/// Faithful port of `MovePassDiagnostic['mode']`'s union.
...
pub fn should_abort_blind_corrections(diagnostics: &[MovePassDiagnostic]) -> bool {
    ...
    last.mode == PassMode::Predicted && second_last.mode == PassMode::Predicted
}
...
mod tests {
    ...
    fn diag(pass: u32, mode: PassMode) -> MovePassDiagnostic {
        MovePassDiagnostic {
            pass,
            mode,
            detected_at: (0.0, 0.0),
    ...
    fn should_abort_blind_corrections_false_on_the_first_predicted_pass() {
        let ds = [diag(0, PassMode::Motion), diag(1, PassMode::Predicted)];
    ...
    fn should_abort_blind_corrections_true_after_2_consecutive_predicted_passes() {
        let ds = [
            diag(0, PassMode::Motion),
            diag(1, PassMode::Predicted),
            diag(2, PassMode::Predicted),
        ];
        assert!(should_abort_blind_corrections(&ds));
    }
    ...
    fn should_abort_blind_corrections_a_template_recovered_pass_resets_the_streak() {
        let ds = [
            diag(0, PassMode::Motion),
            diag(1, PassMode::Predicted),
            diag(2, PassMode::Template),
            diag(3, PassMode::Predicted),
        ];
        assert!(!should_abort_blind_corrections(&ds));
    }
    ...
    fn should_abort_blind_corrections_false_if_last_pass_is_verified() {
        let ds = [
            diag(0, PassMode::Predicted),
            diag(1, PassMode::Predicted),
            diag(2, PassMode::Motion),
        ];
        assert!(!should_abort_blind_corrections(&ds));
    }
    ...
    fn pick_bail_pass_returns_minus_one_when_final_detection_exists() {
        let ds = [
            diag_at(0, PassMode::Motion, 30.0, (750.0, 800.0)),
            diag_at(1, PassMode::Motion, 25.0, (755.0, 805.0)),
        ];
    ...
    fn pick_bail_pass_returns_minus_one_when_final_null_but_only_predicted_passes_exist() {
        let ds = [diag(0, PassMode::Predicted), diag(1, PassMode::Predicted)];
    ...
    fn pick_bail_pass_bails_to_verified_pass_when_final_is_null() {
        let ds = [
            diag_at(0, PassMode::Motion, 100.0, (800.0, 800.0)),
            diag_at(1, PassMode::Motion, 50.0, (750.0, 800.0)),
            diag(2, PassMode::Predicted),
        ];
        assert_eq!(pick_bail_pass(&ds, f64::INFINITY), 1);
    }
    ...
    fn pick_bail_pass_picks_the_smallest_residual_verified_pass_when_multiple_exist() {
        let ds = [
            diag(0, PassMode::Motion),
            diag_at(1, PassMode::Template, 30.0, (755.0, 830.0)),
            diag_at(2, PassMode::Shape, 50.0, (0.0, 0.0)),
            diag(3, PassMode::Predicted),
        ];
        assert_eq!(pick_bail_pass(&ds, f64::INFINITY), 1);
    }
    ...
    fn pick_bail_pass_treats_motion_template_shape_modes_equivalently_as_verified() {
        let ds = [
            diag_at(0, PassMode::Motion, 50.0, (0.0, 0.0)),
            diag_at(1, PassMode::Template, 70.0, (0.0, 0.0)),
            diag_at(2, PassMode::Shape, 30.0, (0.0, 0.0)),
        ];
    ...
    fn pick_bail_pass_ignores_predicted_mode_passes_even_when_residual_is_small() {
        let ds = [
            diag_at(0, PassMode::Predicted, 5.0, (0.0, 0.0)),
            diag_at(1, PassMode::Motion, 30.0, (0.0, 0.0)),
        ];
        assert_eq!(pick_bail_pass(&ds, f64::INFINITY), 1);
    }
    ...
    fn pick_bail_pass_returns_the_first_occurrence_on_ties() {
        let ds = [
            diag_at(0, PassMode::Motion, 30.0, (1.0, 1.0)),
            diag_at(1, PassMode::Motion, 30.0, (2.0, 2.0)),
        ];
