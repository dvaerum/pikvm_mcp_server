...
{ repo }:
let
  pkgs = (builtins.getFlake (toString repo)).inputs.nixpkgs.legacyPackages.aarch64-linux;
  inherit (pkgs) lib;
  ...
  onnxruntime-xnnpack = (pkgs.onnxruntime.override {
    # aarch64-only, round 3: mold (added above) DID fix the main
    # libonnxruntime.so link OOM — confirmed, that step succeeded this
    # attempt. The OOM just moved one output further: linking
    # onnxruntime_pybind11_state.so (the Python bindings' native module,
    # which links AGAINST the already-built libonnxruntime.so) also OOM'd
    # under mold. But per the design doc (§2), the Rust port doesn't need
    # the Python wheel at all — `ort`'s load-dynamic feature only needs
    # $out/lib/libonnxruntime.so, which this build produces regardless of
    # pythonSupport. nixpkgs' own package.nix defaults pythonSupport to
    # `stdenv.buildPlatform.canExecute stdenv.hostPlatform` — true here
    # since this is a native aarch64-linux build — so turning it off both
    # fixes this OOM (skips the failing link entirely) and removes a
    # chunk of irrelevant build work.
    pythonSupport = false;
  }).overrideAttrs (old: {
    ...
    cmakeFlags = old.cmakeFlags ++ [
      ...
      # to it.
      # aarch64-only addition (not in the proven x86_64 overlay): this
      # repo's linux-builder VM is a real 1-core/~2.9GB-RAM/zero-swap
      # aarch64-linux VM (confirmed via a diagnostic remote build reading
      # /proc/meminfo), much smaller than the "beefier shared host" the
      # x86_64 build ran on. First two real attempts (--cores 4, then
      # --cores 1) both got cc1plus OOM-killed at the identical spot
      # (core/util/math_cpu.cc.o) regardless of parallelism, confirming
      # this is a single-TU peak-memory ceiling, not a concurrency issue.
      # --param ggc-min-expand/--param ggc-min-heapsize tune GCC's garbage
      # collector to run more aggressively during compilation, trading
      # compile time for peak memory — does NOT change -O2 codegen or the
      # resulting binary, so a legitimate mitigation for a benchmark build
      # (unlike lowering -O, which would taint the timing numbers this
      # build exists to produce).
      (lib.cmakeFeature "CMAKE_CXX_FLAGS" "-Wno-error=unused-variable -Wno-error=maybe-uninitialized --param ggc-min-expand=10 --param ggc-min-heapsize=32768")
      # aarch64-only, round 2: the ggc-min-expand/ggc-min-heapsize tuning
      # above fixed the COMPILE-time OOM (confirmed — this attempt got all
      # the way through every .cc.o, including math_cpu.cc.o, plus re2 and
      # the generated_source.c.o, 100% built). The OOM moved to the FINAL
      # LINK step instead: `ld` (GNU bfd, nixpkgs' default) got killed
      # (exit 137) linking the single giant libonnxruntime.so, which
      # statically pulls in XNNPACK/kleidiai/onnx/re2/abseil/protobuf all
      # at once — bfd ld's peak RSS for a link this size routinely exceeds
      # what compiling any single TU needs. mold is a drop-in linker
      # replacement built specifically to use far less peak memory (and be
      # much faster) for exactly this kind of huge-C++-binary link,
      # without changing the produced binary's behavior/codegen — same
      # "environment mitigation, not a codegen change" bar as the
      # ggc-* flags above.
      (lib.cmakeFeature "CMAKE_EXE_LINKER_FLAGS" "-fuse-ld=mold")
      (lib.cmakeFeature "CMAKE_SHARED_LINKER_FLAGS" "-fuse-ld=mold")
      (lib.cmakeFeature "CMAKE_MODULE_LINKER_FLAGS" "-fuse-ld=mold")
    ];
