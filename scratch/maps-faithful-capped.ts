...
async function main() {
  ...
  for (let i = 0; i < 15; i++) {
    ...
    let corrected = 'no';
    if (mid.pos && midRes > 30 && midRes < 80) { corrected = `yes(${midRes.toFixed(0)})`; await oneShot(MAPS, mid.pos); } else if (mid.pos && midRes >= 80) { corrected = `SKIP-implausible(${midRes.toFixed(0)})`; }
    const fin = await v8();
