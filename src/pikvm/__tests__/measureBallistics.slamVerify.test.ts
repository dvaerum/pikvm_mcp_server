...
// 400×300 resolution → slamToCorner's own auto-computed call count:
// ceil(max(400,300)/100) + 8 = 4 + 8 = 12. cursor-anchor.ts's migration
// (2026-08-24) removed MeasureBallisticsOptions.slamCalls/nudgeCalls
// entirely — anchorCursor owns both now, unconditionally, so this is no
// longer configurable per-call the way it used to be (see ballistics.ts's
// MeasureBallisticsOptions doc for the full removal rationale).
const SLAM_CALLS = 12;
...
describe('measureCell slam verifyMotion', () => {
  it('rejects the cell WITHOUT measuring when slam motion does not verify (frozen/all-blank frames)', async () => {
    ...
    // Early exit: slam(12) + verifyMotion's own confirmation nudge(1) = 13.
    // nudgeFromEdge / warm-up probe / callsPerCell must NOT have run —
    // cursor-anchor.ts's anchorCursor skips the post-slam nudge entirely
    // when verification fails, same as measureCell's own pre-migration
    // early-return did.
    expect(client.moveCalls).toHaveLength(SLAM_CALLS + 1);
  });
  ...
  it('proceeds to measure normally when slam motion DOES verify (cluster appears near the corner)', async () => {
    ...
    // Full pipeline ran: slam(12) + verify-nudge(1) + nudgeFromEdge(5,
    // anchorCursor's default) + warm-up probe(1) + callsPerCell(3) = 22.
    // Strictly more than the early-exit case's 13 — proves the slam-check
    // did not reject early.
    const early = SLAM_CALLS + 1;
    expect(client.moveCalls.length).toBeGreaterThan(early);
    expect(client.moveCalls).toHaveLength(SLAM_CALLS + 1 + 5 + 1 + 3);
  });
});
